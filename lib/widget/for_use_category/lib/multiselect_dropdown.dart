...
class _MultiSelectDropDownState extends State<MultiSelectDropDown> {
  ...
  void didUpdateWidget(covariant MultiSelectDropDown oldWidget) {
    ...
    if (listEquals(widget.selectedOptions, oldWidget.selectedOptions) == false) {
      debugPrint('didUpdateWidget: ${widget.selectedOptions}, ${oldWidget.selectedOptions}');
  ...
  OverlayEntry _buildOverlayEntry() {
    ...
    final bool isScrollable = widget.dropdownHeight < currentHeight && widget.dropdownHeight > 0;
...
class MultiSelectController extends ValueNotifier<_MultiSelectController> {
  ...
  /// get enabled options
  List<ValueItem> get enabledOptions => value._options.where((element) => !value._disabledOptions.contains(element)).toList();
